...
import { Bot, MessageSquare, Activity, Zap, CalendarCheck, TrendingUp, Clock } from "lucide-react";
import {
  AreaChart,
  Area,
  XAxis,
    ...
    <Layout>
      ...
      <main className="flex-1 p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm text-slate-500 font-medium">Bookings This Month</p>
            ...
            {loading ? (
              ...
              <p className="text-3xl font-bold text-slate-900 tabular-nums">{overview?.totalBookingsThisMonth ?? 0}</p>
            )}
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm text-slate-500 font-medium">Conversion Rate</p>
              <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-teal-50 text-teal-500">
            ...
            {loading ? (
              ...
              <p className="text-3xl font-bold text-slate-900 tabular-nums">{overview?.conversionRate ?? "0.0%"}</p>
            )}
            <p className="text-xs text-slate-400 mt-1">Bookings ÷ conversations</p>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3">
            ...
            {loading ? (
              ...
              <p className="text-3xl font-bold text-slate-900 tabular-nums">{formatHour(overview?.peakHour ?? null)}</p>
            )}
            <p className="text-xs text-slate-400 mt-1">Most active time of day</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height={210}>
                <AreaChart data={chartData} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
                  ...
                  <Tooltip
                    contentStyle={{ backgroundColor: "#0f172a", border: "none", borderRadius: "8px", fontSize: "12px", color: "#e2e8f0", padding: "8px 12px" }}
                    itemStyle={{ color: "#a5b4fc" }}
                    labelStyle={{ color: "#94a3b8", marginBottom: "2px" }}
                  />
                  <Area type="monotone" dataKey="count" name="Conversations" stroke="#6366f1" strokeWidth={2} fill="url(#colorConvos)" dot={false} activeDot={{ r: 4, fill: "#6366f1", strokeWidth: 0 }} />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            ...
            {loading ? (
              <div className="h-52 bg-slate-50 rounded-lg animate-pulse" />
            ) : bookingChartData.every((d) => d.count === 0) ? (
              <div className="h-52 flex flex-col items-center justify-center text-center">
                <CalendarCheck className="w-8 h-8 text-slate-200 mb-3" />
                <p className="text-sm text-slate-400 font-medium">No bookings yet</p>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={210}>
                <AreaChart data={bookingChartData} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
                  <defs>
                  ...
                  <Tooltip
                    ...
                    labelStyle={{ color: "#94a3b8", marginBottom: "2px" }}
                  />
                  <Area type="monotone" dataKey="count" name="Bookings" stroke="#8b5cf6" strokeWidth={2} fill="url(#colorBookings)" dot={false} activeDot={{ r: 4, fill: "#8b5cf6", strokeWidth: 0 }} />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
